(function () {
  ...
  function renderInline(text) {
    ...
    html = html.replace(/\[([^\]]+)\]\(([^)\s]+)\)/g, (_, t, u) => `<a href="${u}" target="_blank" rel="noopener">${t}</a>`);
      ...
      <article class="news-detail">
        <aside class="news-detail-aside">
          ...
          <div class="news-detail-actions">
            ...
            ${link ? `<a class="news-post-discuss" href="${link}" target="_blank" rel="noopener">💬 ${escapeHtml(discussLabel)}</a>` : ''}
          </div>
        </aside>
